...
import { Text, View,KeyboardAvoidingView, 
  TouchableOpacity, TextInput,
   Image, StyleSheet,ToastAndroid,Alert} from 'react-native';
...
checkStudentEligibilityForBookIssue = async()=>{
  ...
  } else { studentRef.docs.map(doc => { var student = doc.data();
     if (student.numberOfBooksIssued < 2)
      { isStudentEligible = true; }
     else { isStudentEligible = false; 
      Alert.alert("The student has already issued 2 books!");
       this.setState({ scannedStudentId: "", scannedBookId: "" }); } }); } return isStudentEligible; }
...
{ var isStudentEligible = await this.checkStudentEligibilityForBookIssue();
 if (isStudentEligible) { this.initiateBookIssue();
  ...
  else { var isStudentEligible = await this.checkStudentEligibilityForReturn();
